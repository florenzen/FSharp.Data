...
for row in companies.Rows |> Seq.truncate 5 do
    printfn "%s (%s) — %s, founded %d" row.Symbol row.Security row.``GICS Sector`` row.Founded(* output: 
*)
(**
...
val downloadsByMinorVersion: (string * decimal) array =
  [|("8.2", 29186M); ("8.1", 140309M); ("8.0", 15039M); ("7.0", 6746M)|]*)
(**
...
for product in catalog.Schemas.Product do
    printfn "%s — %s (SKU: %s)" product.Name product.Brand product.Sku(* output: 
...
for row in page.Tables.Table1.Rows do
    printfn "%d: %s" row.Year row.Achievement(* output: 
...
printfn "URL: %s" article.Url(* output: 
...
for row in wiki.Tables.Table1.Rows do
    printfn "%d: %s" row.Year row.Event(* output: 
...
printfn "Article published: %s" wikiMulti.JsonLd.Article.[0].DatePublished(* output: 
